...
import { getEducationProjectsData } from "@/app/utils/services/api/getServices";
import { EducationProjectsData } from "@/app/utils/types/data";
...
const fetchProjectsData = cache(
  async (): Promise<Partial<EducationProjectsData>> => {
    try {
      const projectsData = await getEducationProjectsData();
      return projectsData || {};
    } catch (error) {
...
export const generateMetadata = async (): Promise<Metadata> => {
  try {
    ...
    return {
      ...
      description,
      alternates: {
        canonical: process.env.NEXT_PUBLIC_BASE_DOMAIN + "/projects/academic",
      },
      openGraph: {
        title: ogTitle,
...
const page = async () => {
  ...
  return (
    <div className="bg-white dark:bg-[#353535] flex flex-col md:overflow-hidden">
      ...
      />
      <AcademicProjects projectsData={projectsData?.data} />
